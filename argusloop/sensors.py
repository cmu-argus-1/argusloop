...
class Gyroscope:
    ...
    def measure(self, spacecraft):
        measurement = (
            self.offset @ spacecraft.angular_velocity
            + self.std_noise * np.random.randn(3)
            + self.bias
            + self.bias_std_noise * np.random.randn(3)
        )
        return measurement
...
class SunVector:
    ...
    def eclipse_state(self, spacecraft):
        ...
        nu = True
        if (
            s_proj / np.linalg.norm(s_proj) >= 1.0
            or np.linalg.norm(spacecraft.orbit_eci[:3] - s_proj * r_sun_eci_normalized) > R_EARTH
        ):
            nu = False
        return nu
